...
RUN chmod +x gradlew
RUN --mount=type=cache,target=/root/.gradle ./gradlew clean build --parallel --configure-on-demand --watch-fs --no-daemon --no-build-cache --refresh-dependencies --no-configuration-cache
